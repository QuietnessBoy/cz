package com.zhjl.tech.common.notify.czxq;
...
public class NotifyFileCzXq {
    ...
    public void notifyChannel(CreateAttestDetailMessage createAttestDetailMessage,String sign,String callBackUrl, int count ) {
        if( count <= 0){
            //重试多次而不成功
            throw new AttestBizException("续期存证完成通知回调失败，ordersn="+createAttestDetailMessage.getOrdersn());
        }
        ...
        try (CloseableHttpClient httpclient = HttpClients.createDefault()){
            ...
            if(response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                log.info("续期存证回调成功.ordersn={}",createAttestDetailMessage.getOrdersn());
            }else{
                //不管何种原因，均需要重新开始回调
                log.warn("续期存证回调失败.ordersn={}",createAttestDetailMessage.getOrdersn());
                count--;
            ...
            throw new AttestBizException("续期存证完成因URISyntaxException回调失败，ordersn="+createAttestDetailMessage.getOrdersn());
        } catch (IOException e) {
            log.error("获取服务器数据失败.ordersn={}",createAttestDetailMessage.getOrdersn(), e);
            throw new AttestBizException("续期存证完成因IOException回调失败，ordersn="+createAttestDetailMessage.getOrdersn());
        }
    }
}
