...
@Slf4j
public class FileCzRequestSign {
    ...
    public static String gen(AttestDto attestDto, String publicKey) {
        log.debug("收到的存证请求参数:attestDto:{},publick:{}",
                JSONObject.toJSONString(attestDto),publicKey);
        ...
        log.debug("存证请求参数的bytes:{}", Hex.encode(pbytes));
    ...
    public static String encode(AttestDto attestDto){
        ...
        parameters.put("fileAddr", StringUtils.isBlank(attestDto.getFileAddr())?"": attestDto.getFileAddr());
        ...
        log.info("生成文件存证请求待签名的字符串:{}",finalParams);
        return finalParams;
    }
}
