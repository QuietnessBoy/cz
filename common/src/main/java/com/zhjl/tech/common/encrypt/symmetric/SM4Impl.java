...
public class SM4Impl implements SM4 {
    ...
    public static final int ROUND=32; //轮数
    public static final int BLOCK=16;//块大小字节 16*8=128bit
     ...
     */
    @Override
    public void encryptStream(InputStream inputStream, OutputStream outputStream, byte[] key ) throws IOException {
        SMS4KeyExt(key,round_key,ENCRYPT);

        byte[] in = new byte[BLOCK];
     ...
     */
    @Override
    public void decryptStream(InputStream inputStream, OutputStream outputStream, byte[] key ) throws IOException {
        ...
        while ((len=inputStream.read(in)) != -1){
            if( len != BLOCK ){
                throw new RuntimeException("sm4解密过程中，输入流的长度不是16字节的证书倍，余数 :" + len);
            }

            //延迟写入
            if( out_write != null ){
                outputStream.write(out_write);
            }
        ...
        //最后一个字节的写入
        int paddings = out_write[15]; //添加的长度
        for( int i = 0; i < BLOCK - paddings;i++){
            outputStream.write(out_write[i]);
        }
    }
    ...
    public int sms4(byte[] in, int inLen, byte[] key, byte[] out, int CryptFlag)
    {
        ...
        //int[] round_key={0};
        SMS4KeyExt(key,round_key,CryptFlag); //生成轮密钥
        byte[] input; //分组的128bit
        byte[] output = new byte[16];//分组的128bit

        while(inLen>=BLOCK)
        {
            input= Arrays.copyOfRange(in, point, point+16);//取出16个字节
    ...
    private int[] round_key = new int[ROUND];
     ...
     * @param Key 16长度的加密密钥
     * @param rk 轮密钥。 长度32
     * @param CryptFlag 是否DECRYPT解密状态
     */
    private void SMS4KeyExt(byte[] Key,int[] rk,int CryptFlag)
    {
        int r,mid;
        int[] x= new int[4];
        int[] tmp =new int[4];//每个都可以存储4字节
        for(int i=0;i<4;i++)
        {
